...
// The Arduino GFX v1.4.9 RGB LCD implementation is not compatible with ESP32 Core v3.x
// This needs to be updated to a newer version of Arduino GFX or the RGB LCD feature disabled
...
// Force ESP32 Arduino Core v3.x path - the old v2.x path doesn't compile with current ESP-IDF
#if 0  // Disabled: (!defined(ESP_ARDUINO_VERSION_MAJOR)) || (ESP_ARDUINO_VERSION_MAJOR < 3)
...
#else
  // RGB LCD functionality disabled - return NULL to indicate unavailable
  // This allows compilation to succeed while the RGB LCD API is being fixed
  return nullptr;
#endif
}
